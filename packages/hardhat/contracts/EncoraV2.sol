...
/// @title EncoraV2
/// @notice Privacy-preserving content marketplace with FHE-encrypted analytics.
///
/// PRIVACY MODEL:
///   - hasPaid: public (needed for access gating)
///   - purchaseCount per content: FHE-encrypted (only seller can decrypt)
///   - purchasesByBuyer: REMOVED (no public purchase history)
///   - Seller analytics: FHE-encrypted, only seller can unseal
///
/// ENCRYPTION MODEL:
///   Layer 1 (off-chain): Seller AES-GCM encrypts full text → IPFS CID
///   Layer 2 (on-chain):  Seller FHE-encrypts the 32-byte AES key as 8 × euint32 chunks
contract EncoraV2 is ReentrancyGuard {
    // ─────────────────────────────────────────────
    // TYPES
    // ─────────────────────────────────────────────
    ...
    // Purchase count per content — only seller can decrypt
    ...
    function uploadContent(
        ...
        // Initialize encrypted purchase count to 0
        purchaseCount[id] = FHE.asEuint32(0);
        FHE.allowThis(purchaseCount[id]);
        FHE.allow(purchaseCount[id], msg.sender); // only seller can decrypt
    ...
    /// @notice Pay for content. Purchase count is FHE-encrypted — only seller sees it.
    function purchase(uint256 contentId) external {
        ...
        // Increment encrypted purchase count (only seller can see)
    ...
    /// @return sealedChunks The 8 euint32 handles — buyer unseals with their permit
    ...
    // ─────────────────────────────────────────────
    // FHE ANALYTICS (seller-only)
    // ─────────────────────────────────────────────

    /// @notice Get encrypted purchase counts for your content. Only seller can unseal.
    /// @param contentIds Array of content IDs owned by msg.sender
    /// @return counts Array of euint32 handles — unseal client-side with cofhejs
